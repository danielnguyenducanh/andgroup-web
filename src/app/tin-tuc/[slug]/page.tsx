...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const post = await getInsightBySlug(slug, 'vi');
  if (!post) return {};
  return {
    title: post.meta_title || post.title,
    description: post.meta_description || post.excerpt || undefined,
    alternates: { canonical: `https://andgroup.com.vn/tin-tuc/${slug}` },
    openGraph: {
      title: post.meta_title || post.title,
      description: post.meta_description || post.excerpt || undefined,
...
// Strip figure ảnh cover lặp ở đầu content (pipeline nhúng wp-image-cover).
...
export default async function TinTucDetail({ params }: PageProps) {
  const { slug } = await params;
  const post = await getInsightBySlug(slug, 'vi');
  if (!post) notFound();
  ...
  const jsonLd = {
    ...
    mainEntityOfPage: { '@type': 'WebPage', '@id': `https://andgroup.com.vn/tin-tuc/${slug}` },
  };
  const breadcrumbLd = {
    '@context': 'https://schema.org',
    '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'Trang chủ', item: 'https://andgroup.com.vn' },
      { '@type': 'ListItem', position: 2, name: 'Tin tức & Sự kiện', item: 'https://andgroup.com.vn/tin-tuc' },
      { '@type': 'ListItem', position: 3, name: post.title, item: `https://andgroup.com.vn/tin-tuc/${slug}` },
    ],
  };
      ...
      <main style={{ background: 'var(--bg-primary)' }}>
        ...
        <div style={{
          ...
          <div style={{ maxWidth: '820px', margin: '0 auto' }}>
            <Link href="/tin-tuc" style={{ color: 'var(--gold)', fontSize: '0.8rem', fontWeight: 600, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '0.35rem', marginBottom: '1.5rem' }}>
              ← Tin tức & Sự kiện
            </Link>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', flexWrap: 'wrap' }}>
              ...
              <span style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>{readTime} phút đọc</span>
            </div>
          </div>
        </div>
